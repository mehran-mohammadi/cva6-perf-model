...
class TAGE:
    ...
    @dataclass
    class Entry:
        "An entry in a TAGE predictor table"
        valid: bool = False
        tag: int = 0  # Partial tag for match checking
        sat_counter: int = 0  # 3-bit signed saturating counter for prediction
        useful_counter: int = 0  # 2-bit counter for usefulness tracking

    def __init__(self, entries=128, num_components=4, history_lengths=None, tag_bits=9, addr_shift=1):
        self.base_predictor = [TAGE.Entry() for _ in range(entries)]
        self.base_size = entries  # Size of the base predictor

        # Create tagged components with progressively smaller table sizes
        self.tagged_components = []
        self.table_sizes = []  # Store sizes for each tagged component
        for i in range(num_components):
            table_size = entries // (2 ** i)  # Example: smaller tables for longer histories
            self.tagged_components.append([TAGE.Entry() for _ in range(table_size)])
            self.table_sizes.append(table_size)

        self.history_lengths = history_lengths or [2, 4, 8, 16]
        self.global_history = 0  # Global History Register
        self.tag_bits = tag_bits
        self.addr_shift = addr_shift

    def predict(self, addr):
        "Predict if the branch is taken"
        # Base predictor
        base_index = self._index(addr, 0, self.base_size)
        base_entry = self.base_predictor[base_index]

        # Check tagged components
        for i, table in enumerate(self.tagged_components):
            index = self._index(addr, self.history_lengths[i], self.table_sizes[i])
            entry = table[index]
            if entry.valid and entry.tag == self._tag(addr, self.history_lengths[i]):
                # Found a matching entry, return its prediction
                return entry.sat_counter >= 0

        # No matching tagged entry, use the base predictor
        return base_entry.sat_counter >= 0

    def resolve(self, addr, taken):
        "Update branch prediction and global history"
        base_index = self._index(addr, 0, self.base_size)
        base_entry = self.base_predictor[base_index]

        # Update the base predictor
        self._update_counter(base_entry, taken)

        # Check tagged components
        provider = None
        altpred = base_entry.sat_counter >= 0

        for i, table in enumerate(self.tagged_components):
            index = self._index(addr, self.history_lengths[i], self.table_sizes[i])
            entry = table[index]
            if entry.valid and entry.tag == self._tag(addr, self.history_lengths[i]):
                provider = (entry, index, i)
                altpred = entry.sat_counter >= 0
                break

        # Update provider's prediction and usefulness
        if provider:
            entry, index, i = provider
            self._update_counter(entry, taken)
            if taken != altpred:
                entry.useful_counter = min(entry.useful_counter + 1, 3)

        # Allocate a new entry if mispredicted
        if not provider or (taken != (provider[0].sat_counter >= 0)):
            self._allocate(addr, taken, provider)

        # Update the global history
        self.global_history = ((self.global_history << 1) | int(taken)) & ((1 << max(self.history_lengths)) - 1)

    def _allocate(self, addr, taken, provider):
        "Allocate a new entry in a tagged component if needed"
        for i, table in enumerate(self.tagged_components):
            if provider and i <= provider[2]:
                continue  # Only allocate to tables with longer history
            index = self._index(addr, self.history_lengths[i], self.table_sizes[i])
            entry = table[index]
            if entry.useful_counter == 0:  # Found a candidate for replacement
                entry.valid = True
                entry.tag = self._tag(addr, self.history_lengths[i])
                entry.sat_counter = 1 if taken else -1  # Initialize weakly correct
                entry.useful_counter = 0
                break

    def _index(self, addr, history_length, table_size):
        "Calculate index using address, history, and specific table size"
        history_segment = self.global_history & ((1 << history_length) - 1)
        return ((addr >> self.addr_shift) ^ history_segment) % table_size

    def _tag(self, addr, history_length):
        "Generate a partial tag for the entry"
        history_segment = self.global_history & ((1 << history_length) - 1)
        return ((addr >> self.addr_shift) ^ history_segment) & ((1 << self.tag_bits) - 1)

    def _update_counter(self, entry, taken):
        "Update the saturating counter in the entry"
        if taken:
            entry.sat_counter = min(entry.sat_counter + 1, 3)
        else:
            entry.sat_counter = max(entry.sat_counter - 1, -3)
...
class Model:
    ...
    def __init__(
        ...
        self.tage = TAGE(entries=128, num_components=4, history_lengths=[2, 4, 8, 16], tag_bits=9)
        self.instr_queue = []
        self.scoreboard = []
    ...
    def predict_branch(self, instr):
        ...
        pred = self.tage.predict(instr.address)
        if pred is not None:
            return pred
    ...
    def commit_manage_last_branch(self, instr, cycle):
        ...
        if self.last_committed is not None:
            ...
            if last.is_branch():
                ...
                branch_miss = taken != (self.tage.predict(last.address) or False)
                ...
                self.tage.resolve(last.address, taken)
